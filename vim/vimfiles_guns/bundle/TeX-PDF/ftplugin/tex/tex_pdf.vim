...
function! <SID>BuildTexPdf(view_results, ...)
    ...
    if filereadable("Makefile")
        ...
        if executable(l:special_tex_compiler)
            ...
            echon "compiling ..."
            let b:tex_flavor = exists('g:tex_flavor') ? g:tex_flavor : 'pdflatex'
            compiler tex
            execute "setlocal makeprg=".b:tex_flavor."\\ \\-file\\-line\\-error\\ \\-interaction=nonstopmode\\ \\-output\\-directory=".shellescape(expand('%:p:h'))."\\ $*\\\\\\|\\ grep\\ \\-P\\ ':\\\\d{1,5}:\\ '"
            setlocal errorformat=%f:%l:\ %m
            silent make %
        endif
    endif
...
function! <SID>ViewTexPdf(...)
    ...
    if has('mac') || g:VIM_PLATFORM == 'macunix'
