...
if !hasmapto('<Plug>HexManager')
  map <unique> <Leader>xm <Plug>HexManager
endif
if !hasmapto('<Plug>HexDelete')
  map <unique> <Leader>xd <Plug>HexDelete
endif
if !hasmapto('<Plug>HexInsert')
  map <unique> <Leader>xi <Plug>HexInsert
endif
if !hasmapto('<Plug>HexGoto')
  map <unique> <Leader>xg <Plug>HexGoto
endif
if !hasmapto('<Plug>HexNext')
  map <unique> <Leader>xn <Plug>HexNext
endif
if !hasmapto('<Plug>HexPrev')
  map <unique> <Leader>xp <Plug>HexPrev
endif
if !hasmapto('<Plug>HexToggle')
  map <unique> <Leader>xt <Plug>HexToggle
endif
if !hasmapto('<Plug>HexStatus')
  map <unique> <Leader>xs <Plug>HexStatus
endif
if !hasmapto('<Plug>HexFind')
  map <unique> <Leader>xf <Plug>HexFind
endif
...
" 
  ...
  echo ""
  echo "	<leader>xm		Hex(manager) toggle on / off"
  echo "	<leader>xd  		Delete hex character under cursor"
  echo "	<leader>xi  		Insert ascii character before cursor"
  echo "	<leader>xg  		Goto hex offset. "
  echo "	<leader>xf  		Find Hex Character."
  echo "	<leader>xn  		Goto next hex offset. "
  echo "	<leader>xp  		Goto previous hex offset. "
  echo "	<leader>xt  		Switch cursor between hex and ascii area "
  echo "	<leader>xs  	        Show / Hide hexoffset infos in statusline"
