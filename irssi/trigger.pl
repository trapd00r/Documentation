...
If you can give a list, seperate elements by space and use quotes around the list.
     ...
     -except: %|The event must not be in one of the given channels
...
What to do when it matches:
     ...
     -later: %|let irssi completely handle the event first before executing the command
...
Other options:
     ...
     -stripped: %|Remove all color codes before attempting to match the message text
...
# "event "<cmd>, SERVER_REC, char *args, char *sender_nick, char *sender_address
{
	...
	'sub' => sub {
		...
		my (@modeargs) = defined($modeargs) ? split(/ /,$modeargs) : ();
...
#sub strip_colors {
#	my ($s) = @_;
#	$s =~ s!\cC[0-9]{1,2}(?:,(?:[0-9]{1,2})?)?|[\cB\cV\c_\cO]|\cD(?:[/-?]{1,2}|[`-i])|\e\[(?:[0-9]{1,3}(?:,[0-9]{1,3})*)?m!!g;
#	$s
#}

# list of all switches
my @trigger_switches = (@trigger_types, qw(all nocase stop once debug disabled stripped later));
...
sub check_signal_message {
	...
	my ($changed, $stopped, $need_rebuild);
...
TRIGGER:	
	foreach my $trigger (@{$triggers_by_type{$condition}}) {
		...
		if ($trigger->{'command'}) { # it's a (nonempty) -command
			...
			my $context = $server && ($channelname && $server->channel_find($channelname) || $server);
			my $cmd = $context ? sub { $context->command("eval $command") } : sub { Irssi::command("eval $command") };
			if ($trigger->{'later'}) {
				Irssi::timeout_add_once(10, $cmd, undef);
			} else {
				$cmd->();
			}
		}
...
sub compile_trigger {
	...
	if ($trigger->{'regexp'}) {
		...
		$regexp =~ s/(.)/$mask_to_regexp{$1} || quotemeta $1/eg;
	} else {
...
sub cmd_load {
	...
	my $converted = 0;
	open my $io, '<', $trigger_file or do {
		Irssi::print("Error opening triggers file $trigger_file: $!", MSGLEVEL_CLIENTERROR);
		return;
	};
	@triggers = ();
	my $text = readline $io;
	...
	if ($file_version lt '0.6.1+2') {
		no strict 'vars';
		$text .= $_ foreach readline $io->getlines;
		my $rep = eval "$text";
		if (! ref $rep) {
		...
		for (my $index=0;$index < scalar(@old_triggers);$index++) { 
			...
			if ($file_version lt '0.6.1+1' && $trigger->{'modifiers'}) {
				if ($trigger->{'modifiers'} =~ /i/) {
					$trigger->{'nocase'} = 1;
					Irssi::print("Trigger: trigger ".($index+1)." had 'i' in its modifiers, it has been converted to -nocase");
				}
			...
			$trigger = parse_options({},@args);
			if ($trigger) {
				push @triggers, $trigger;
			}
		}
	} else { # new format
		while ( $text = readline $io ) {
			chomp $text;
			next if $text =~ /^\s*(?:#|$)/;
			my @args = &shellwords($text . ' a');
...
ARGS:	for (my $arg = shift @args; $arg; $arg = shift @args) {
	...
	if ($trigger->{stripped} && !$trigger->{regexp} && !$trigger->{pattern}) {
		Irssi::print("Trigger error: Can't have -stripped without -regexp", MSGLEVEL_CLIENTERROR);
		return undef;
	}
